...
import resut.utils.EDCoder;
import resut.utils.Reflect;
import resut.utils.XLog;
...
//        insertResManager(); //可能有危險。。。比如影響到其他本身就需要多apk包的應用的正常使用 , 不使用；通過加入spliteResDir[] 讓所有new 的res 加入 ext
        ...
        System.out.println("============ AppExtResManager 3 attachBaseContext（）  end  use time:" + (e - s));
    }
...
//        }
    ...
    public void tryDecodeFile() {
        try {
            ...
            XLog.i(TAG, "tryDecodeFile: lastSize:" + lastSize + "  lastV:" + lastV);
...
//                extractFile(instance);
                ...
                XLog.i(TAG, "tryDecodeFile: nowsize:" + fsize + "  nowV:" + versionCode);
...
//        return pi.versionCode;
    ...
    private long decodeFile() throws Exception {
        //copy + decode
        PackageManager pm = getPackageManager();
        ApplicationInfo ai = pm.getApplicationInfo(getPackageName(), PackageManager.GET_META_DATA);

        long totalSize;
        ExtResConfig erc = new ExtResConfig();
        erc.updateValid(null, null, BuildConfig.ENCA, BuildConfig.ENCR);
//        readBuffer
        File baseFile = new File(ai.publicSourceDir);

        su = new StreamUtils();
        try (OutputStream bofs = openFileOutput(extFileName, MODE_PRIVATE);
            ...
            totalSize = copyZipContent(inZipFile, zipInputStream, zipOutputStream, erc);
        } catch (Exception e) {
            throw e;
        } finally {
            su.destory();
    ...
    long copyZipContent(ZipFile inZipFile, final ZipInputStream zis, final ZipOutputStream zos, ExtResConfig config) throws Exception {
        ZipEntry zisEntry;
        long totalSize = 0;
        ...
        while (entries.hasMoreElements()) {
            ...
            XLog.d("copyZipContent zisEntry.getName() " + eName);
            ...
            if ("resources.arsc".equals(eName)) {
                ...
                if ((eName.startsWith(config.encResDirPath))) {
                    ...
                    int surfixLen = (eName.length() - eName.lastIndexOf('.'));
                    outeName = eName.substring(config.encResDirPath.length(), eName.length() - surfixLen);
                    XLog.d(TAG, "copyZipContent:1 outeName " + outeName);
                    outeName = config.orgResDirPath + new String(su.getEdCoder().decode(Base64.decode(outeName, Base64.DEFAULT)));
                    XLog.d(TAG, "copyZipContent:2 outeName " + outeName);
            ...
            if (isSkip) {
                XLog.d("copyZipContent skip " + eName);
                continue;
            }

            totalSize += processEntry(zos, zisEntry, eName, outeName, singleFileIs, isDoDecode);
        }
        zos.finish();
    ...
    private long processEntry(ZipOutputStream zos, ZipEntry zisEntry, String eName, String outeName, InputStream singleFileIs, boolean isDoDecode) throws Exception {
        ...
        int orgFileSize = -1;
        if (isDoDecode) {
            orgFileSize = su.decodeCopy(singleFileIs, zos, zosEntry, zisEntry);
        } else {
            orgFileSize = su.storedCopy(singleFileIs, zos, zosEntry, zisEntry);
        }
        XLog.d("copyZipContent eName->outeName: " + eName + "-> " + outeName + " decode:" + isDoDecode + " totalSize " + orgFileSize);
        return orgFileSize;
    }

    private class StreamUtils {
        private byte[] readBuffer;
        private EDCoder edCoder;

        StreamUtils() {
            init();
        }

        public EDCoder getEdCoder() {
            return edCoder;
        }

        public void destory() {
            readBuffer = null;
            edCoder.releaseDecodeCipher();
            edCoder = null;
        }
        ...
        public int decodeCopy(InputStream inputStream, ZipOutputStream zos, ZipEntry zosEntry, ZipEntry zisEntry) throws Exception {
            Cipher cipher = edCoder.getDecodeCipher();
            int offset = 0;
            int len = 0;
            int orgFileSize = 0;
            while ((len = inputStream.read(readBuffer, offset, readBuffer.length - offset)) != -1) {
                offset += len;
                readBuffer = arrayGrowth(readBuffer, offset);
            }
            byte[] decodedBuf = cipher.doFinal(readBuffer, 0, offset);

            XLog.d("beforDecodeLen: " + offset);
//        XLog.d("beforDecodeBase64: " + new String(Base64.encode(b,0,offset, Base64.DEFAULT)));
//        decoded = EDCoder.decode(decoded, 0, outputOffset);
            XLog.d("afterDecodeLen: " + decodedBuf.length);

//        if(ZipEntry.STORED == zisEntry.getMethod()){
            zosEntry.setMethod(ZipEntry.STORED);
            zosEntry.setSize(decodedBuf.length);
            CRC32 crc32 = new CRC32();
            crc32.update(decodedBuf, 0, decodedBuf.length);
            zosEntry.setCrc(crc32.getValue());
//        }
            zos.putNextEntry(zosEntry);
            zos.write(decodedBuf, 0, decodedBuf.length);
            zos.closeEntry();
            orgFileSize = offset;
            return orgFileSize;
        }

        public byte[] arrayGrowth(byte[] b, int offset) {
            if (offset == b.length) {
        ...
        public int storedCopy(InputStream inputStream, ZipOutputStream zos, ZipEntry zosEntry, ZipEntry zisEntry) throws IOException {
            int offset = 0;
            int len = 0;
            int orgFileSize = 0;
            while ((len = inputStream.read(readBuffer, offset, readBuffer.length - offset)) != -1) {
...
//        if(ZipEntry.STORED == zisEntry.getMethod()){
            zosEntry.setMethod(ZipEntry.STORED);
            zosEntry.setSize(offset);
            CRC32 crc32 = new CRC32();
            crc32.update(readBuffer, 0, offset);
            zosEntry.setCrc(crc32.getValue());
//        }
            zos.putNextEntry(zosEntry);
            zos.write(readBuffer, 0, offset);
            zos.closeEntry();
            orgFileSize = offset;
            return orgFileSize;
        }
